...
type Show = {
  ...
  inner?: Show        // what the view inside a wrapper shows
}
```
...
asks for that part alone; `inner` is what a wrapper's inner view shows. A
`view` that is not registered is ignored and the rules decide.
