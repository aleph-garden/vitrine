...
| `VIEW_ATTR`, `IRI_ATTR` | the attributes the runtime writes on every region it holds: the outermost view's id and the resource. `wrap` writes `VIEW_ATTR` on the body of each layer below with that layer's view |
...
| `subjectsGridView`, `subjectsListView`, `sourceView`, `fallbackView`, `containerView`, `folderView`, `fileRowView` | the built-in views. `folderView` draws a container with each member as a transcluded row, which `fileRowView` draws unless a rule names another; a host registers both, and a rule or a show picks `folderView` |
