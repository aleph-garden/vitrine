...
type Context = {
  ...
  inner(show?: Show, resource?: Resource): Promise<Rendered & { view: View }>
...
views](/vitrine/docs/author/wrappers/) has it. Given a `resource`, it draws that
one instead, which is how a view hands on a graph it derived; [Compose
views](/vitrine/docs/author/compose/) has it.
