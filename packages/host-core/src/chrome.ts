 ...
 *  `resolveIssuer`. The IRI on show is mirrored into document.title. */
/** The issuer a WebID names for itself, which the host resolves because
 *  reading the profile needs a Turtle parser. */
export type ResolveIssuer = (webId: string) => Promise<string>

export type Chrome = {
  session: Session
  /** The issuer to log in at; absent means the control asks for a WebID. */
...
export function installChrome(host: Element, chrome: Chrome): void {
  const { session, issuer, runtime, credentialed, address, resolveIssuer } = chrome
  ...
  const login = loginControl(session, issuer, resolveIssuer)
  ...
  const show = () => {
    const shown = address.of(location.href)
    const url = new URL(shown.iri)
    hostName.textContent = url.hostname
    iri.textContent = shown.iri
    dot.dataset.state = credentialed(url.origin) ? 'reaches' : 'anonymous'
    document.title = `${shown.iri} · Aleph Garden`
  }
  show()
...
function loginControl(
  ...
  form.addEventListener('submit', (event) => {
    event.preventDefault()
    void loginAs(form, session, resolveIssuer, input.value)
  })
  return form
}
...
async function loginAs(
  ...
  try {
    await session.login?.(await resolveIssuer(webId))
  } catch (e) {
