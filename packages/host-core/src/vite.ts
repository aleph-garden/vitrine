// The build-time end of the host document. readConfig reads it out of the
// page; these put it in, so both ends of the format stay in one package.

import { readFileSync } from 'node:fs'
import type { Plugin } from 'vite'

const EMPTY_HOST = '{"@context":"https://aleph.garden/ns/vitrine-context.jsonld","@type":"Host"}'
...
export function hostDocument(): Plugin {
  return {
    name: 'aleph-host',
    transformIndexHtml() {
...
export function documentNote(): Plugin {
  return {
    name: 'aleph-note',
