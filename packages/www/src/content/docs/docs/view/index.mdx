...
library. Everything a view can do, it does through these. Concrete views
and concrete hosts are implementations under it and are documented where
they live.
...
A host is whatever answers `resolve`, places the HTML the library returns,
and drives the re-render protocol. Everything the library refuses to decide
is its: fetching, session, navigation, the address, editing, search, layout.
A view never fetches, never touches the address bar, never reaches outside
its own region.

The repository ships one host, a browser shell. A Solid server hands it out
as the `text/html` representation of every resource it serves, and the same
bundle runs at `https://aleph.garden/` for any IRI written after the origin.
What it configures, how it logs in and how it is deployed belong to that
shell; the contract on this page holds whatever a host does with it.

The library does not model what a view is beyond the signature above: every
case arrives as a concrete view under the same contract.
