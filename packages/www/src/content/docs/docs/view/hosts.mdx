...
aleph.garden's document opens every IRI, registers two views and pins the
landing view to the site's own IRI:
...
is the chrome's control. Any other failed mount leaves the message and a
link that opens the resource at its own origin, which a foreign resource
...
Credentials follow that line. The shell fetches with the session on its own
origin, the WebID's origin, the issuer's origin, and the origins the WebID's
profile names as `solid:oidcIssuer` and `pim:storage`, so a visitor reaches
their own pod through aleph.garden. That profile is read once a tab. Every
other IRI is fetched without credentials, since a DPoP token names the
visitor and their issuer to whoever receives it. Where the session stops,
the chrome marks the resource on show `anonymous here` next to the WebID,
and a `401` there leaves a sentence in the region that names the host and
offers the resource at its own origin, where the login can happen.
