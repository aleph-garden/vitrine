...
that fetch for three origins: the shell's own, the WebID's and the
issuer's. Every other IRI is fetched without credentials.
...
origin, the WebID's origin and the issuer's origin, and reaches every other
IRI without them, since a DPoP token names the visitor and their issuer to
whoever receives it. A private resource elsewhere therefore asks for a login
the shell cannot give it, and the region says so.
