...
`writeHtml` is the one place view HTML enters the document. It sanitizes
against a fixed allowlist and, where the browser has Trusted Types, produces
the `TrustedHTML` the sink takes. A view returns a string and never writes
its region; a view that writes DOM in `hydrate` calls `writeHtml` for the
same allowlist. [Foreign content](/docs/view/hosts/#foreign-content) has the
...
`as:View` with the link's IRI as `object` and the full URL as `target`. One
declarative rule says which clicks stay with the browser: an anchor carrying
a `target` or `download` attribute, and a click made with a modifier key or a
non-primary button. A view marks the links the browser should follow by
writing those attributes and needs no code for it.
