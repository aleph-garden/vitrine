...
`iri` pins a view to one resource, whatever that resource contains. The
shell's landing page is its first use, through a rule in the host document
of `https://aleph.garden`:
