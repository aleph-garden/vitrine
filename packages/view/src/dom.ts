...
import { AS, type Context, type Event, type Handle, type Hint, type Renderer, type Resource } from "./index.ts";
...
export function createRuntime(renderer: Renderer, resolve: Resolve): Runtime {
  ...
  const dispatch = async (event: Event): Promise<void> => {
    for (const inst of [...live.values()]) {
      ...
      if (inst.handle?.update) {
        ...
        if (!patch) continue;
        const slot = patch.slot === undefined ? inst.region : inst.region.querySelector(`[data-slot="${patch.slot}"]`);
        if (slot) slot.innerHTML = patch.html;
        continue;
      }
      if (event.type === AS.Update && event.object !== undefined) {
        if (event.object === inst.iri || inst.deps.has(event.object)) await rerender(inst, inst.hint);
      } else if (event.type === AS.View && event.object === inst.iri) {
        const hint = hintFrom(event, inst.hint);
        if (hint.fragment !== inst.hint?.fragment || hint.view !== inst.hint?.view) await rerender(inst, hint);
      }
    }
  };
  ...
  const mount = async (region: Element, iri: string, hint?: Hint): Promise<Instance> => {
    ...
    const { ctx, dependencies } = instanceContext(resolve, (e) => queueMicrotask(() => void dispatch(e)), queue.iterable);
    const inst = {
      id,
      iri,
      hint,
