...
function matchesType(expected: string | RegExp, contentType: string): boolean {
  ...
  return typeof expected === "string" ? expected.toLowerCase() === mediaType : expected.test(mediaType);
}
...
export const containerView: View = {
  ...
  async render(resource) {
    const items = objects(resource.meta, resource.iri, LDP_CONTAINS).map((child) => {
      ...
      const time = modified ? ` <time datetime="${escapeHtml(modified)}">${escapeHtml(modified)}</time>` : "";
...
function termHtml(term: Term): string {
  if (term.termType === "NamedNode") return `<a href="${escapeHtml(term.value)}">${escapeHtml(term.value)}</a>`;
